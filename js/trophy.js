...
var trophyState = {
  ...
  create: function () {
    ...
    this.title = game.add.text(1100, 100, "Trophées", {
      font: fontxl,
      fill: textColor,
    });
    game.add
    ...
    for (let j = 0; j < 3; j++) {
      for (let i = 0; i < 5; i++) {
        this.trophyLocked[this.trophyCounter] = game.add.sprite(
          840 + j * 200,
          200 + i * 60,
          "trophy"
        );
        ...
        game.add
          .tween(this.trophyLocked[this.trophyCounter])
          .to({ x: 140 + j * 200 }, 400, Phaser.Easing.Linear.Out, true);

        this.trophyLockedText[this.trophyCounter] = game.add.text(
          865 + j * 200,
          200 + i * 60,
          this.trophyData.trophyNames[this.trophyCounter],
        ...
        game.add
          .tween(this.trophyLockedText[this.trophyCounter])
          .to({ x: 165 + j * 200 }, 400, Phaser.Easing.Linear.Out, true);
  ...
  menu: function () {
    if (
      !this.muteButton.input.pointerOver() &&
      this.returnButton.input.pointerOver()
    ) {
  ...
  transitionAnimation: function () {
    game.add
      .tween(this.title)
      .to({ x: this.title.x - 800 }, 400, Phaser.Easing.Linear.Out, true);
    game.add
      .tween(this.returnButton)
      .to(
    ...
    for (let i = 0, j = this.trophyCounter; i < j; i++) {
      game.add
        .tween(this.trophyLocked[i])
        .to(
          { x: this.trophyLocked[i].x - 800 },
          400,
          Phaser.Easing.Linear.Out,
          true
        );
      game.add
        .tween(this.trophyLockedText[i])
        .to(
          { x: this.trophyLockedText[i].x - 800 },
          400,
          Phaser.Easing.Linear.Out,
          true
        );
